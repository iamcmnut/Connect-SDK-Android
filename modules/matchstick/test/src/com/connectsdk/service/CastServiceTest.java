...
@PrepareForTest(FlintManager.class)
public class CastServiceTest {

    CastService service;

    FlintManager googleApiClient = PowerMockito.mock(FlintManager.class);
    ...
    @Before
    public void setUp() {
        service = new StubCastService(mock(ServiceDescription.class),
                mock(ServiceConfig.class));
    ...
    public void testConnectOnlyOnce() {
        // Test desc.: if service is already connected it shouldn't invoke
        // connect
    ...
    public void testSeek() {
        // Test desc.: only if googleApi is connected and media player state is
        // not null should invoke seek method
        ...
        service.mApiClient = googleApiClient;
        when(
                mediaPlayer.seek(googleApiClient, position,
                        RemoteMediaPlayer.RESUME_STATE_UNCHANGED)).thenReturn(
        ...
        // then
        verify(mediaPlayer).seek(googleApiClient, position,
    ...
    public void testSeekWithEmptyMediaState() {
        ...
        ArgumentCaptor<ServiceCommandError> errorArgument = ArgumentCaptor
                .forClass(ServiceCommandError.class);
        verify(mediaPlayer, times(0)).seek(googleApiClient, position,
                RemoteMediaPlayer.RESUME_STATE_UNCHANGED);
        verify(listener).onError(errorArgument.capture());

        Assert.assertEquals("There is no media currently available",
                errorArgument.getValue().getMessage());
    }
    ...
    public void testGetPosition() {
        // Test desc.: should call getApproximateStreamPosition method and
        // onSuccess
